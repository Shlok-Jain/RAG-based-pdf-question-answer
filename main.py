...
def should_use_document(query, llm, prev):
    prompt = f'''
...
def generate(query, vectorstore, llm, prev_conversation):
    # query = input("You: ")
    if should_use_document(query, llm, prev_conversation):
    ...
    prev_conversation += f"Question: {query}\nAnswer: {response}\n"
    return response

def send_message(user_input, vectorstore, llm, prev_conversation):
    user_message = st.session_state.user_input
    if user_message:
        st.session_state.messages.append({'role': 'user', 'text': user_message})
        
        response = generate(user_message, vectorstore, llm, prev_conversation)
...
def main():
    ...
    if "prev_conversation" not in st.session_state:
      st.session_state.prev_messages = prev_conversation
    st.set_page_config(
    ...
    st.text_area("Your message:", key='user_input', on_change=lambda: send_message(st.session_state.user_input, vectorstore, llm, prev_conversation))
    st.button("Send", on_click=lambda: send_message(st.session_state.user_input, vectorstore, llm, prev_conversation))
